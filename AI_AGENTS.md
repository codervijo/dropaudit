# AI Agent Context — dropaudit.co

## Summary

*one paragraph: what this site is, what it does*

DropAudit is a compliance SaaS that automates the mandatory DROP (Delete Request and Opt-Out Platform) workflow for California-registered data brokers under SB 361. It connects to the CalPrivacy DROP platform on the required 45-day schedule, downloads incoming deletion requests, runs fuzzy-matching against the broker's database, routes exemption decisions, assigns the required status codes, submits response reports back to CalPrivacy, and maintains a full audit log. Data brokers stop doing this manually and stop accumulating $200-per-day-per-request penalty exposure.

## Audience

*one sentence: who this is for (broad demographic)*

Privacy engineers, legal counsel, and compliance officers at California-registered data brokers who must process DROP deletion requests every 45 days starting August 1, 2026.
...
A privacy engineer or legal ops manager at a 10–500 person company registered as a data broker with the California Privacy Protection Agency. Has read the SB 361 penalty structure, done the math on $200/day per unfulfilled request, and realized manual processing is not viable at scale. Decision cycle is days not weeks — the August 1 enforcement deadline is the forcing function. Will buy without a demo if the compliance guarantee is clear.

## Goals

*1-2 sentences: primary business / product goal*

Sign every California-registered data broker before the August 1, 2026 enforcement deadline using the public CPPA registry as a cold outreach list, and become the default DROP compliance infrastructure as other states adopt similar deletion mandate frameworks.
...
`proj=dropaudit.co`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=dropaudit.co` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=dropaudit.co` | Start the dev server, skipping install. |
| `make test proj=dropaudit.co` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=dropaudit.co
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy dropaudit.co"` runs `gh repo create` and
...
- **Live URL:** https://dropaudit.co/  *(update once first deploy succeeds)*
...
Landing page targeting SB 361 compliance software and DROP platform data broker automation. Docs covering the full DROP workflow, exemption categories, status codes, and response report format. Blog: SB 361 penalty exposure calculator as a standalone SEO tool, step-by-step DROP integration guide for engineers, comparison of manual vs automated compliance cost, timeline explainer for the August 1 deadline. Format mix: one definitive technical guide, penalty calculator as lead magnet, FAQ pages targeting zero-KD bill-number queries, and outreach-ready one-pagers for the law firm referral channel.

### Post-deploy checklist (do these once after the first successful deploy)

- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:dropaudit.co` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://dropaudit.co/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status dropaudit.co` resolves cleanly).
...
`portfolio project check dropaudit.co` (run from `sites/portfolio/`).
