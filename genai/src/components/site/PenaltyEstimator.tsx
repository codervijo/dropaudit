...
export function PenaltyEstimator({ source = "homepage_estimator" }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const { error } = await supabase.from("penalty_estimator_leads").insert({
        email: parsed.data.email,
        company: parsed.data.company ?? null,
        unresolved_requests: parsed.data.unresolved_requests,
      ...
      toast.error("Couldn't submit right now. Please try again in a moment.");
    } finally {
      setSubmitting(false);
    }
  }
